...
import (
	...
	"github.com/catwashere/microservice/cmd/three_microservices_crypto/user/envs"
...
func main() {
	logrus.Info("Initializing User microservice")
	...
	f1, err := os.ReadFile("./id_rsa.pem")
	if err != nil {
		logrusFields := logrus.Fields{"error": err.Error()}
		logrusMessage := "Error loading public key"
		logrus.WithFields(logrusFields).Fatal(logrusMessage)
	}

	publicKey, err := crypto.FileBytesToPublicKey(f1)
	if err != nil {
		logrusFields := logrus.Fields{"error": err.Error()}
		logrusMessage := "Error loading public key"
		logrus.WithFields(logrusFields).Fatal(logrusMessage)
